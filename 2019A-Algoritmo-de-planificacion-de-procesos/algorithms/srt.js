...
module.exports = function srt(inputProcesos, print = false){
  ...
  for (let time = 0; time < obtenerMaximoTimeline(inputProcesos); time++) {
    ...
    // Verificamos la cola de procesos y obtenemos al que le felta menos tiempo para ejecutarse
    queue.forEach(proceso => {
      if ((proceso.t - proceso.metadata.indexesTimeline.length) < (procesoActual.t - procesoActual.metadata.indexesTimeline.length)){
